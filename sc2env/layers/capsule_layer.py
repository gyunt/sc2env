...
class CapsuleLayer(base.Layer):
    ...
    def call(self, inputs):
        ...
        self.W = tf.expand_dims(self.W, axis=0)
        self.W = tf.tile(self.W, multiples=[batch_size, 1, 1, 1, 1])
        assert self.W.get_shape().is_compatible_with(
            [None,
        ...
        B = self.B
        U = inputs
        U_hat = self.get_u_hat(U, self.W)
        assert (U_hat.get_shape().is_compatible_with([None, self.previous_num_capsule, self.num_capsule, self.vec_len]))
    ...
    def get_u_hat(U, W):
        with tf.variable_scope('UHat'):
            num_capsule = W.get_shape()[2]
            U = tf.expand_dims(U, axis=2)
            U = tf.expand_dims(U, axis=2)
            U = tf.tile(U, multiples=[1, 1, num_capsule, 1, 1])
            U_hat = tf.squeeze(tf.matmul(U, W), axis=3)
            return U_hat
